...
1. 下载并解压 `PS-Sezhao-Photoshop-Developer-v0.3.2.zip`。
...
1. 下载并解压 `PS-Sezhao-LightroomClassic-macOS-arm64-v0.3.2.zip`。
2. 完全退出 Lightroom Classic。
3. 解压后删除或移走旧的 `PS-Sezhao.lrplugin` 文件夹，避免新旧 Lua 文件混用。
4. 重新打开 Lightroom Classic，进入 `文件 → 增效工具管理器`。
5. 点击“添加”，选择新的 `PS-Sezhao.lrplugin` 文件夹。
6. 从 `图库 → 增效工具额外功能 → PS-Sezhao：转正所选负片` 启动。

v0.3.2 修复了 `AgExportSession:addRenditionsForPhotos: must not call on main UI task`。安装后可在增效工具管理器中确认版本为 0.3.2。

### Windows x64

步骤相同，但下载 `PS-Sezhao-LightroomClassic-Windows-x64-v0.3.2.zip`。
...
1. 下载并解压 `PS-Sezhao-Standalone-macOS-arm64-v0.3.2.zip`。
2. 将 `PS-Sezhao.app` 移到“应用程序”。
3. 首次启动若被 Gatekeeper 阻止，请在 Finder 中右键应用并选择“打开”。

### Windows x64

1. 下载并解压 `PS-Sezhao-Standalone-Windows-x64-v0.3.2.zip`。
2. 运行 `PS-Sezhao.exe`。
...
- 插件清单最低版本为 `25.0.0`。
- v0.3.1 起已移除需要 Photoshop 25.10 才支持的模态超时选项。
